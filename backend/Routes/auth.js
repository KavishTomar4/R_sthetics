...
router.post('/register', async(req, res)=>{
    ...
    try{
        ...
        res.cookie('rs_client', token, {maxAge: 30*24*60*60, httpOnly: true, secure: true, sameSite: 'none'}).send("cookie sent");
...
router.get('/logout', (req, res)=>{
    res.cookie('rs_client', '', {maxAge: 1, httpOnly: true, secure: true, sameSite: 'none'}).send("cookie sent");
    res.json({toLink: '/'})
})
